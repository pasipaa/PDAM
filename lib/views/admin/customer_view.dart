...
class _CustomerViewState extends State<CustomerView> {
  ...
  void initState() {
    ...
    _customerController = Provider.of<CustomerAdminController>(context, listen: false);
  ...
  Color _getServiceColor(String serviceName) {
    ...
    if (name.contains('rumah') || name.contains('tetap')) return const Color(0xff0066FF);
    if (name.contains('komersial') || name.contains('bisnis')) return Colors.orange;
    ...
    return Scaffold(
          ...
          children: [
            Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: Colors.blueGrey.shade100.withOpacity(0.6), width: 1.5),
                ),
            ...
            SizedBox(
              ...
              child: ListView(
                ...
                children: ["Semua", "Rumah Tangga", "Komersial", "Industri"].map((filter) {
            ...
            Expanded(
              child: Consumer<CustomerAdminController>(
                builder: (context, controller, child) {
                  ...
                  final filteredList = controller.customers.where((item) {
                    final serviceName = (item.service?.name ?? "Layanan Tetap").toLowerCase();
                    final nameMatches = item.name.toLowerCase().contains(searchC.text.toLowerCase());

                    bool filterMatches = activeFilter == "Semua";
                    if (activeFilter == "Rumah Tangga") {
                      filterMatches = serviceName.contains("rumah") || serviceName.contains("tetap");
                    } else if (activeFilter != "Semua") {
                      filterMatches = serviceName.contains(activeFilter.toLowerCase());
                    }
                  ...
                  if (filteredList.isEmpty) {
                    return Center(
                      child: Text("Belum ada data customer", style: TextStyle(color: Colors.grey.shade500)),
                    );
                  }
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final serviceName = customer.service?.name ?? "Layanan Tetap";
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(customer.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xff0A2540))),
                                    const SizedBox(height: 2),
                                    Text("NIK : ${customer.customerNumber}", style: TextStyle(fontSize: 12, color: Colors.grey.shade500, fontWeight: FontWeight.w500)),
                                    const SizedBox(height: 6),
                                    Row(
                                      children: [
                                        Container(
                                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                                          decoration: BoxDecoration(color: _getServiceColor(serviceName), borderRadius: BorderRadius.circular(6)),
                                          child: Text(serviceName, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
                                        ),
                                        const SizedBox(width: 6),
                                        Container(
                                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                          decoration: BoxDecoration(color: const Color(0xffE6F9F0), borderRadius: BorderRadius.circular(6)),
                                          child: const Text("Aktif", style: TextStyle(color: Color(0xff00B365), fontSize: 10, fontWeight: FontWeight.bold)),
                                        ),
                                      ],
                                    ),
                                  ],
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    constraints: const BoxConstraints(),
                                    padding: const EdgeInsets.all(4),
                                    onPressed: () => _showDeleteBottomSheet(context, controller, customer.id),
                                  ),
                                ],
                              ),
                            ],
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => TambahCustomerView(token: _activeToken)),
          ).then((_) {
            if (mounted) {
              _customerController.getCustomers(_activeToken);
            }
      ...
      bottomNavigationBar: CustomBottomNavbar(token: _activeToken, currentIndex: 2),
    );
  }
  ...
  void _showDeleteBottomSheet(BuildContext context, CustomerAdminController provider, int id) {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      bool success = await provider.deleteCustomer(id, _activeToken);
                      
                      if (context.mounted) {
                        if (success) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Customer berhasil dihapus"), backgroundColor: Colors.green)
                          );
                          provider.getCustomers(_activeToken);
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                ...
                SizedBox(
                  ...
                  child: OutlinedButton(
                    ...
                    child: const Text("Batal", style: TextStyle(color: Colors.black, fontSize: 15, fontWeight: FontWeight.bold)),
                  ),
                ),
              ],
            ),
