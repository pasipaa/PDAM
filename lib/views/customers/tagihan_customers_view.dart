...
class _TagihanCustViewState extends State<TagihanCustView> {
  ...
  Widget _buildStatusButton(String status) {
    final lunasStatuses = ['lunas', 'selesai', 'verified', 'success', 'approved', 'done'];
    final pendingStatuses = ['pending', 'menunggu', 'proses', 'waiting', 'verifying', 'review'];
    ...
    if (lunasStatuses.contains(status)) {
      return OutlinedButton(
        ...
        child: const Text(
          "Selesai (Lunas)", 
          style: TextStyle(color: Color(0xff16A34A), fontWeight: FontWeight.bold, fontSize: 14)
        ),
      );
      ...
      return OutlinedButton(
        ...
        style: OutlinedButton.styleFrom(
          side: const BorderSide(color: Color(0xff0A59D1), width: 1.5),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          backgroundColor: const Color(0xffEFF6FF),
        ),
        child: const Text(
          "Belum Bayar", 
          style: TextStyle(color: Color(0xff0A59D1), fontWeight: FontWeight.bold, fontSize: 14)
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<CustomerBill>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return const Center(
              child: Text("Belum ada data riwayat tagihan.", style: TextStyle(color: Colors.grey))
            );
          }
          ...
          final data = allData.where((item) {
            ...
            final pendingStatuses = ['pending', 'menunggu', 'proses', 'waiting', 'verifying', 'review'];
            final approvedStatuses = ['lunas', 'selesai', 'verified', 'success', 'approved', 'done'];
            final rejectedStatuses = ['ditolak', 'rejected', 'gagal', 'failed'];
      ...
      bottomNavigationBar: CustomCustomerBottomNavbar(token: widget.token, currentIndex: 1),
    );
  }
