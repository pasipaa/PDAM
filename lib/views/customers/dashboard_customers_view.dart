...
class _DashboardCustomerViewState extends State<DashboardCustomerView> {
  ...
  Future<List<CustomerBill>>? _billsFuture;
  ...
  void _refreshData() {
    setState(() {
      ...
      _billsFuture = _controller.getMyBills(widget.token);
    });
  }
      ...
      body: SafeArea(
        child: FutureBuilder<CustomerProfile?>(
          ...
          builder: (context, profileSnapshot) {
            final profileName = profileSnapshot.data?.name ?? "Royyan";

            return FutureBuilder<List<CustomerBill>>(
              future: _billsFuture,
              builder: (context, billsSnapshot) {
                final apiBills = billsSnapshot.data ?? [];
                final unpaidBills = apiBills
                    .where((b) => b.status.toLowerCase() != 'lunas' && b.status.toLowerCase() != 'paid')
                    .toList();
                  ...
                  child: Column(
                    ...
                    children: [
                      ListTile(
                        ...
                        subtitle: Text("Welcome back, $profileName", style: const TextStyle(color: Colors.grey, fontSize: 13)),
                      ),
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                const Text("Tagihan Belum Dibayar", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xff0F172A))),
                            ...
                            Row(
                              ...
                              children: [
                                Text("${unpaidBills.length}", style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Color(0xff0A59D1))),
                                const SizedBox(width: 6),
                                const Text("Tagihan Tertunda", style: TextStyle(color: Colors.grey, fontSize: 14)),
                              ],
                            ),
                            ...
                            if (billsSnapshot.connectionState == ConnectionState.waiting)
                              ...
                              const Text("Lunas 🎉 Semua tagihan Anda sudah terbayar.", style: TextStyle(color: Colors.grey, fontSize: 14))
                            else
                              ListView.separated(
                                shrinkWrap: true,
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text("${_getIndonesianMonth(bill.month)} ${bill.year} Bill", style: const TextStyle(color: Color(0xff64748B), fontSize: 14)),
                                      Text("Rp ${bill.totalPrice}", style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xff1E293B), fontSize: 14)),
                                    ],
                                  );
                                },
                              ),
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                child: Text("Pay Now", style: TextStyle(color: unpaidBills.isEmpty ? Colors.grey.shade500 : Colors.white, fontSize: 15, fontWeight: FontWeight.bold)),
                              ),
                            ),
                          ],
                        ),
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                const Text("Analisis Penggunaan", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xff0F172A))),
                                Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                  decoration: BoxDecoration(color: const Color(0xffEAF2FF), borderRadius: BorderRadius.circular(20)),
                                  child: const Text("Tahun Ini", style: TextStyle(color: Color(0xff0A59D1), fontSize: 12, fontWeight: FontWeight.bold)),
                                )
                              ],
                            ),
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: isSelected ? const Color(0xff0A59D1) : const Color(0xff1A61CC).withOpacity(0.15),
        ...
        Text(label,
            style: TextStyle(
                ...
                color: isSelected ? const Color(0xff0A59D1) : const Color(0xff64748B))),
      ],
    );
  }
  ...
  Widget _buildContainerCard({required Widget child}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: Colors.black.withOpacity(0.02), spreadRadius: 2, blurRadius: 12, offset: const Offset(0, 4))
        ],
      ),
      child: child,
    );
